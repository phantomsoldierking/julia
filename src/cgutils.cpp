                             ...
                             Value *ptindex = nullptr, MDNode *tbaa_ptindex = nullptr)
{
    ...
    if (Order == AtomicOrdering::NotAtomic && !isboxed && (!aliasscope && elty->isAggregateType() && !jl_is_genericmemoryref_type(jltype))) {
        ...
        return mark_julia_slot(copy.first, jltype, nullptr, ctx.tbaa().tbaa_stack, copy.second);
    }
    Type *realelty = elty;
    ...
    if (Order == AtomicOrdering::NotAtomic && !isboxed && jl_is_genericmemoryref_type(jltype)) {
        ...
        return jl_cgval_t(instr, jltype, nullptr);
    }
    else {
    ...
    if (instr)
        ...
        return mark_julia_slot(intcast, jltype, nullptr, ctx.tbaa().tbaa_stack);
}
        ...
        Value *ptindex = nullptr, MDNode *tbaa_ptindex = nullptr)
{
    ...
    bool is_other = !isboxed && !is_union && jl_is_uniontype(jltype);
    ...
    auto store_union = [&](const jl_cgval_t &val, const jl_cgval_t &val_union) {
        ...
        if (!val.isghost)
            emit_unionmove(ctx, ptr, jltype, tbaa, val, /*skip*/nullptr);
    };
    auto load_union = [&]() {
    ...
    Value *r = nullptr;
    jl_value_t *loadtype = jltype;
    if (is_other) {
        assert(Order == AtomicOrdering::NotAtomic);
        maybe_null_if_boxed = false;
    ...
    if (!is_union) {
        ...
        if (isboxed)
            ...
            alignment = julia_alignment(loadtype);
        elty = isboxed ? ctx.types().T_prjlvalue : julia_type_to_llvm(ctx, loadtype);
        if (type_is_ghost(elty) ||
                (op == StoreKind::SetOnce && !maybe_null_if_boxed) ||
                (op == StoreKind::SetOnce && !isboxed && !jl_type_hasptr(loadtype))) {
            if (isStrongerThanMonotonic(Order))
                ctx.builder.CreateFence(Order);
            switch (op) {
            case StoreKind::Set:
            ...
            case StoreKind::Replace: {
                Value *Success = emit_f_is(ctx, cmpop, ghostValue(ctx, loadtype));
                Success = ctx.builder.CreateZExt(Success, getInt8Ty(ctx.builder.getContext()));
                const jl_cgval_t argv[2] = {ghostValue(ctx, loadtype), mark_julia_type(ctx, Success, false, jl_bool_type)};
                jl_datatype_t *rettyp = jl_apply_cmpswap_type(jltype);
                return emit_new_struct(ctx, (jl_value_t*)rettyp, 2, argv);
            }
            case StoreKind::Swap:
                return ghostValue(ctx, loadtype);
            case StoreKind::Modify: {
                jl_cgval_t oldval = ghostValue(ctx, loadtype);
        ...
        nb = isboxed ? sizeof(void*) : jl_datatype_size(loadtype);
    ...
    if (op == StoreKind::Set || (Order == AtomicOrdering::NotAtomic && op == StoreKind::Swap)) {
        ...
        assert(!is_union && !is_other); // union swap handled above in NotAtomic branch
        ...
        if (Order == AtomicOrdering::NotAtomic) {
            ...
            assert(!is_union && !is_other && r); // union requires NotAtomic
            if (Order == AtomicOrdering::Unordered)
                Order = AtomicOrdering::Monotonic;
                                         ...
                                         enum jl_memory_order order, Value **nullcheck)
{
    ...
    bool isother = kind == JL_FIELDKIND_ISOTHER && jl_is_uniontype(jfty);
    bool maybe_null = field_may_be_null(strct, jt, idx);
    if (isother) {
        // override nullcheck to use for tindex instead
        nullcheck = &otherval;
        maybe_null = true;
    }
    ...
    if (!strct.inline_roots.empty()) {
        ...
        if (hasptr) {
            roots = ArrayRef(strct.inline_roots).slice(offsets.second, kind == JL_FIELDKIND_ISPTR ? 1 : ((jl_datatype_t*)jfty)->layout->npointers);
            if (maybe_null)
        ...
        if (jfty == (jl_value_t*)jl_bool_type) {
            ...
            return mark_julia_slot(addr, jfty, tindex, tbaa, roots);
        }
    }
        ...
        if (needlock)
            emit_lockstate_value(ctx, needlock, true);
        if (isother) {
            jfty = ((jl_uniontype_t*)jfty)->b;
            assert(jl_is_concrete_type(jfty) && !jl_is_pointerfree(jfty));
        }
...
static void emit_unionmove(jl_codectx_t &ctx, Value *dest, jl_value_t *desttype,
        MDNode *tbaa_dst, const jl_cgval_t &src, Value *skip, bool isVolatile)
{
        ...
        const jl_cgval_t *modifyop, const Twine &fname)
{
    ...
    enum jl_fieldkind_t kind = jl_field_kind(sty, idx0);
